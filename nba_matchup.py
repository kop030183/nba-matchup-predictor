...
#   3. 一次性呼叫 ESPN 標準排名 API，抓「全聯盟30隊」的戰績/勝率
...
def fetch_espn_team(nba_id, all_standings, season_type, espn_season):
    """
    抓取單一球隊的近況資料，回傳 dict：
      - 整季戰績/主客場戰績/勝率：來自 fetch_standings() 已抓好的資料
      - 近10場戰績、每場得失分、對五成以上/以下球隊戰績：來自賽程 API 逐場計算

    參數：
      nba_id：NBA.com TEAM_ID，查 ESPN_ID 對照表轉成 ESPN 隊伍編號
      all_standings：fetch_standings() 的回傳值；例行賽模式才會用來計算「對五成球隊戰況」
      season_type：ESPN 賽季類型，2=例行賽、3=季後賽
      espn_season：ESPN 用的賽季年份，見 get_current_espn_season()
    """
    eid = str(ESPN_ID.get(nba_id, ''))
    own = all_standings.get(eid, {}) if all_standings else {}
    ...
    # 對五成球隊戰況：只在例行賽模式（all_standings 不為 None）才計算，
    # 且要看整季所有已完賽的比賽，不只是近10場
    if all_standings is not None:
        for ev in completed:
...
SHOW_VS500 = (mode == '1')                 # 季後賽對手勝率必然>=0.5，此統計沒有意義
ESPN_SEASON = get_current_espn_season()

df_adv = fetch_all_nba_stats()
with console.status('[orange1]📡 載入全聯盟戰績...[/orange1]'):
    all_standings = fetch_standings(ESPN_SEASON) if SHOW_VS500 else None
...
while True:
    ...
    with console.status('[orange1]📡 載入戰績數據...[/orange1]'):
        with ThreadPoolExecutor(max_workers=2) as ex:
            fa = ex.submit(fetch_espn_team, idA, all_standings, SEASON_TYPE, ESPN_SEASON)
            fb = ex.submit(fetch_espn_team, idB, all_standings, SEASON_TYPE, ESPN_SEASON)
    ...
    # 估算節奏(pace)：(平均得分+平均失分)/2.3，2.3 是經驗係數
    # （NBA一場比賽雙方合計約200分，回合數約95~100，200/100≈2.0~2.3）
    def est_pace(off, def_):
        return round((float(off) + float(def_)) / 2.3, 1)
    ...
    tables += [
        ...
        ('🔮 預測比分', [
            ...
            # 預測法2「PACE法」：得分效率(得分/pace) x 兩隊平均節奏
            # 概念：若本場節奏落在兩隊近期平均值，A用原本效率大約能拿幾分
            ('PACE法\n(攻守效率 x 平均節奏)',
